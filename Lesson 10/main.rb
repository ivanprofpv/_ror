...
class Main
  ...
  def each_block_wagon_menu
    ...
    train.each_block_wagon do |wagon|
      puts "Тип вагона: #{wagon.type}, производитель: #{wagon.manufacturer},
           свободные места или объем: : #{wagon.free_place},
           заброн.места или объем: #{wagon.reserved_place}"
    end
  end
  ...
  def manage_route
    ...
    when :del
      ...
      elsif route_number.stations.first == station_name || route_number.stations.last == station_name
        puts 'Станции с таким названием нет в промежуточных станциях маршрута!'
      elsif !station_name.trains.empty?
